...
export function fetchSearchResults(searchQuery) {
    const request = axios.get(`https://api.beta.steamlocked.com/search/${searchQuery}`);
...
export function fetchAppInfo(appid) {
    const request = axios.get(`https://api.beta.steamlocked.com/appid/${appid}`)
...
export function fetchList(country) {
    const request = axios.get(`https://api.beta.steamlocked.com/list?billingtype=3,10&country=${country}`);
