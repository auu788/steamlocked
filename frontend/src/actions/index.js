...
export function fetchSearchResults(searchQuery) {
    const request = axios.get(`https://apibeta.steamlocked.com/search/${searchQuery}`);
...
export function fetchAppInfo(appid) {
    const request = axios.get(`https://apibeta.steamlocked.com/appid/${appid}`)
...
export function fetchList(country) {
    const request = axios.get(`https://apibeta.steamlocked.com/list?billingtype=3,10&country=${country}`);
